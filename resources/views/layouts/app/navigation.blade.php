...
@if(can('view_dashboard'))
    <x-nav.link route="admin" icon="fas fa-home">Page d'accueil</x-nav.link>
@endif

@if(can('view_planning'))
    <x-nav.group label="Planning" icon="fas fa-calendar" route="admin.planning">
        <x-nav.link route="admin.planning" route="admin.planning.index">Planning</x-nav.link>
    </x-nav.group>
@endif

@if(can('view_audit_trails') || can('view_sent_emails'))
    <x-nav.group label="Paramètres" route="admin.settings" icon="fas fa-cogs">
...
@if(can('view_users'))
    <x-nav.link route="admin.users.index" icon="fas fa-users">Utilisateurs</x-nav.link>
@endif
